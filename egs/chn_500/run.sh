...
tests="ai_mcsntr_test400 accent_test ailab_tmjl_mangbiao_dec11"
...
if [ $stage -le 7 ]; then
  for set in $tests; do
    ...
    python steps/calculate_logits.py  --nj=1 --input_scp=data/tests_post/${set}/test.scp --output_unit=121 --data_path=$dir --output_dir=exp/decode_$set/ark
  done
fi  
...
if [ $stage -le 8 ]; then  
  ...
  tests="ai_mcsntr_test400 accent_test ailab_tmjl_mangbiao_dec11"
  for set in $tests; do
      bash local/decode.sh $acwt $set $nj || exit 1;
  done
fi

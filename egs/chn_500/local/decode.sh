...
if [ $stage -le 1 ]; then
  $cmd JOB=1:$nj $ark_dir/log/decode.JOB.log \
    /aifs/users/tx078/tools/eesen/src/decoderbin/latgen-faster --max-mem=$max_mem --min-active=$min_active --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam \

...
#python2 utils/ctc_token_fst_corrected.py den data/lang_phn/tokens.txt | fstcompile --isymbols=$dir/tokens.txt --osymbols=$dir/tokens.txt \
#  --keep_isymbols=false --keep_osymbols=false | fstarcsort --sort_type=olabel > $dir/T.fst || exit 1;
...
cat $tmpdir/lexiconp.txt | awk '{print $1}' | uniq  | awk '
  BEGIN {
    print "<eps> 0";
    print "<UNK> 1";
  } 
